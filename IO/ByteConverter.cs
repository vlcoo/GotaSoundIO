...
public abstract class ByteConverter
{
    ...
    /// <summary>
    ///     Gets a <see cref="T:Syroot.BinaryData.ByteConverter" /> instance converting data stored in little endian byte
    ///     order.
    /// </summary>
    ...
    /// <summary>
    ///     Gets a <see cref="T:Syroot.BinaryData.ByteConverter" /> instance converting data stored in the byte order of the
    ///     system
    ...
    public static ByteConverter GetConverter(ByteOrder byteOrder)
    {
        ...
        if (byteOrder == ByteOrder.LittleEndian)
            return LittleEndian;
        throw new ArgumentException(string.Format("Invalid {0}.", "ByteOrder"), nameof(byteOrder));
    }
    ...
    public void GetBytes(decimal value, byte[] buffer, int startIndex = 0)
    {
        ...
        {
            var index2 = startIndex + index1 * 4;
            var num = bits[index1];
    ...
    /// <summary>
    ///     Returns an <see cref="T:System.Decimal" /> instance converted from the bytes in the given
    ...
    public decimal ToDecimal(byte[] buffer, int startIndex = 0)
    {
        ...
        {
            var index2 = startIndex + index1 * 4;
            bits[index1] = buffer[index2] | (buffer[index2 + 1] << 8) | (buffer[index2 + 2] << 16) |
                           (buffer[index2 + 3] << 24);
        }
