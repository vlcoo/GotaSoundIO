...
public abstract class SoundFile : IOFile
{
    ...
    public void FromOtherStreamFile(SoundFile other, int targetBlockSize = -2)
    {
        ...
        foreach (var t in other.Tracks) Tracks.Add(t.Duplicate());
        ...
        {
            Audio.Convert(PreferredEncoding(), targetBlockSize == -2 ? Audio.BlockSize : targetBlockSize,
                Loops ? (int)LoopStart : -1, Loops ? (int)LoopEnd : -1);
            AfterConversion();
            return;
        }
        ...
        Audio.Convert(SupportedEncodings()[0], targetBlockSize == -2 ? Audio.BlockSize : targetBlockSize,
            Loops ? (int)LoopStart : -1, Loops ? (int)LoopEnd : -1);
    ...
    public void FromOtherStreamFile(SoundFile other, Type audioEncoding, int targetBlockSize = -2)
    {
        ...
        //Set channel data.
        Audio.Convert(audioEncoding, targetBlockSize == -2 ? Audio.BlockSize : targetBlockSize,
            Loops ? (int)LoopStart : -1, Loops ? (int)LoopEnd : -1);
    ...
    {
        var ret = (SoundFile)Activator.CreateInstance(targetType);
        ret.FromOtherStreamFile(this);
        return ret;
    }
    ...
    public void AlignLoopToBlock(uint blockSamples)
    {
        ...
        if (LoopStart % blockSamples != 0)
        {
            ...
            var dist1 = LoopStart / blockSamples * blockSamples;
            var dist2 = (LoopStart / blockSamples + 1) * blockSamples;
            var backward = Math.Abs(dist1 - LoopStart) < Math.Abs(dist2 - LoopStart);
    ...
    {
        if (Loops || LoopEnd != 0) Audio.Trim((int)LoopEnd + 1);
    }
    ...
    public class TrackData
    {
        ...
        {
            var t = new TrackData();
            foreach (var c in Channels) t.Channels.Add(c);
            foreach (var p in Properties) t.Properties.Add(p.Key, new TrackProperty(p.Value.Type, p.Value.Data));
            return t;
        }
    }
