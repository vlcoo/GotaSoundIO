...
public class ImaAdpcm : IAudioEncoding
{
    ...
    {
        var e = new ImaAdpcmEncoder(pcm, out Sample, out Index);
        Data = e.Encode();
    }
    ...
    {
        var d = new ImaAdpcmDecoder(Sample, Index, Data);
        return d.Decode();
    }
    ...
    public List<IAudioEncoding> ChangeBlockSize(List<IAudioEncoding> blocks, int newBlockSize)
    {
        ...
        var a = new AudioData { Channels = new List<List<IAudioEncoding>> { blocks } };
    ...
    public T GetProperty<T>(string propertyName)
    {
        ...
        if (propertyName.ToLower().Equals("index")) return (T)(object)Index;
        return default;
    }
    ...
    public void SetProperty<T>(T value, string propertyName)
    {
        ...
        else if (propertyName.ToLower().Equals("index")) Index = (int)(object)value;
    }
    ...
    {
        var ret = new ImaAdpcm { Data = new byte[Data.Length] };
...
public class ImaAdpcmDecoder
{
    ...
    public float[] Decode()
    {
        ...
        while (Offset < Data.Length) ret.Add((float)GetSample() / short.MaxValue);
        return ret.ToArray();
    }
    ...
    {
        var sample = GetSample((byte)((Data[Offset] >> (SecondNibble ? 4 : 0)) & 15));
        if (SecondNibble)
            ++Offset;
        SecondNibble = !SecondNibble;
        return sample;
    ...
    {
        Sample = ImaAdpcmMath.ClampSample(Sample +
...
public class ImaAdpcmEncoder
{
    ...
    public byte[] Encode()
    {
        var data = new byte[Data.Length / 2 + (Data.Length % 2 != 0 ? 1 : 0)];
        var secondNibble = false;
        var dataPtr = 0;
        for (var i = 0; i < Data.Length; i++)
        {
            var config = GetBestConfig(Index, ConvertFloat(Data[i]) - Sample);
            Sample = ImaAdpcmMath.ClampSample(Sample +
                                              (ImaAdpcmMath.StepTable[Index] / 8 +
    ...
    private int GetBestTableIndex(int diff)
    {
        ...
        var num2 = -1;
        for (var index = 0; index < ImaAdpcmMath.StepTable.Length; ++index)
        {
            var num3 = Math.Abs(Math.Abs(diff) - ImaAdpcmMath.StepTable[index]);
            if (num3 < num1)
            {
                num1 = num3;
                num2 = index;
    ...
    private int GetBestConfig(int index, int diff)
    {
        ...
        if (diff < 0)
            num1 |= 8;
        diff = Math.Abs(diff);
        var num2 = ImaAdpcmMath.StepTable[index] / 8;
        ...
        {
            num1 |= 1;
            var num3 = num2 + ImaAdpcmMath.StepTable[index] / 4;
        }
