...
public class AudioData
{
    ...
    /// </summary>
    public int DataSize => Channels.Count == 0
        ? 0
        : Channels[0].Select(x => x.DataSize()).Sum() + LastBlockPaddingSize * (Channels.Count - 1);
    ...
    public AudioData Duplicate()
    {
        ...
        {
            var chan = new List<IAudioEncoding>();
            for (var j = 0; j < Channels[i].Count; j++) chan.Add(Channels[i][j].Duplicate());
            a.Channels.Add(chan);
        }
    ...
    public void Convert(Type targetEncoding, int targetBlockSize, int loopStart = -1, int loopEnd = -1)
    {
        ...
        for (var i = 0; i < Channels.Count; i++)
        {
            ...
            foreach (var b in Channels[i]) samples.AddRange(b.ToFloatPCM(decodingData));
            ...
            //Temporary block.
            var tmp = (IAudioEncoding)Activator.CreateInstance(targetEncoding);
            ...
            var samplesPerBlock = tmp.SamplesFromBlockSize(targetBlockSize);
            BlockSamples = samplesPerBlock;
            var numBlocks = samples.Count / samplesPerBlock;
            if (samples.Count % samplesPerBlock != 0) numBlocks++;
            for (var j = 0; j < numBlocks; j++)
            {
                var numSamples = j == numBlocks - 1 ? samples.Count % samplesPerBlock : samplesPerBlock;
                ...
                var block = (IAudioEncoding)Activator.CreateInstance(targetEncoding);
    ...
    public void ChangeBlockSize(int targetBlockSize)
    {
        ...
        BlockSize = targetBlockSize;
        for (var i = 0; i < Channels.Count; i++) Channels[i] = tmp.ChangeBlockSize(Channels[i], targetBlockSize);
        if (targetBlockSize == -1)
            BlockSamples = -1;
        else
    ...
    public void Read(FileReader r, Type encodingType, int numChannels, int dataSize, int numSamples, int dataPadding)
    {
        ...
        for (var i = 0; i < numChannels; i++)
        {
            ...
            var data = (IAudioEncoding)Activator.CreateInstance(EncodingType);
    ...
    public void Read(FileReader r, Type encodingType, int numChannels, uint numBlocks, uint blockSize,
        uint blockSamples, uint lastBlockSize, uint lastBlockSamples, uint lastBlockPaddingSize)
    {
        //Set properties.
        EncodingType = encodingType;
        ...
        //Init channels.
        Channels.Clear();
        for (var i = 0; i < numChannels; i++) Channels.Add(new List<IAudioEncoding>());
    ...
    public void SetSeek(byte[] seekInfo)
    {
        ...
        using (var src = new MemoryStream(seekInfo))
        {
    ...
    {
        //Trim samples.
        var samplesToTrim = NumSamples - totalSamples;
        for (var i = 0; i < Channels.Count; i++)
        {
            var toTrim = samplesToTrim;
            while (toTrim > 0)
            {
                var cutSamples = Math.Min(Channels[i].Last().SampleCount(), toTrim);
                Channels[i].Last().Trim(totalSamples);
                toTrim -= cutSamples;
                if (Channels[i].Last().SampleCount() == 0) Channels[i].Remove(Channels[i].Last());
            }
        }
    }
    ...
    public void MixToMono(bool[] mutes = null)
    {
        ...
        //Get mutes.
        if (mutes == null) mutes = new bool[Channels.Count];

        //Test.
        if (Channels.Count == 0 || mutes.Where(x => x == false).Count() == 0) return;

        //Divisor to make audio not loud.
        var divisor = 1 / Math.Sqrt(mutes.Where(x => x == false).Count());
        ...
        for (var i = 0; i < NumBlocks; i++)
        {
            ...
            for (var j = 0; j < Channels[0][i].SampleCount(); j++)
            {
                double sample = 0;
                for (var k = 0; k < Channels.Count; k++)
                    if (!mutes[k])
    ...
    public void MixToStereo(bool[] mutes = null, bool[] isRightChannel = null, bool[] isBoth = null)
    {
        ...
        if (isBoth == null)
        {
            ...
            if (Channels.Count % 2 != 0) isBoth[Channels.Count - 1] = true;
        }
        ...
        //Get divisors.
        var divL = 1 / Math.Sqrt(lefts.Count);
        var divR = 1 / Math.Sqrt(rights.Count);

        //Start mixing.
        var left = new List<IAudioEncoding>();
        var right = new List<IAudioEncoding>();
        for (var i = 0; i < NumBlocks; i++)
        {
            var samplesL = new List<float>();
            var samplesR = new List<float>();
            ...
            for (var j = 0; j < Channels[0][i].SampleCount(); j++)
            {
                ...
                for (var k = 0; k < Channels.Count; k++)
                    if (!mutes[k])
                    {
                        if (lefts.Contains(k)) sampleL += Channels[k][i].ToFloatPCM()[j];
                        if (rights.Contains(k)) sampleR += Channels[k][i].ToFloatPCM()[j];
                    }
